...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="SimpleSerial driver for Delvaux Roulette masked-intermediate kernel")
    p.add_argument("--hex", required=True)
    p.add_argument("--no-program", action="store_true")
    p.add_argument("--label", default="delvaux-roulette")
    ...
    p.add_argument("--const-value", type=parse_int, default=0)
    p.add_argument("--bit-mask", type=parse_int, default=1)
    p.add_argument("--rand-seed", type=parse_int, default=0x12345678)
...
def configure_fault(target, args) -> Dict[str, int]:
    ...
    payload[3] = 0
    payload[4:6] = u16(args.const_value)
    payload[6:8] = u16(args.bit_mask)
    payload[8:12] = u32(args.rand_seed)
    ...
    d = {
        ...
        "const_value": int.from_bytes(r[4:6], "little"),
        "bit_mask": int.from_bytes(r[6:8], "little"),
        "rand_seed": int.from_bytes(r[8:12], "little"),
        "ncoeffs": int.from_bytes(r[12:14], "little"),
        "raw": r.hex(),
    }
    if d["ret"] != 0:
...
def read_status(target, timeout, verbose) -> Dict[str, int]:
    ...
    return {
        ...
        "target_expected_value": int.from_bytes(r[8:12], "little"),
        "target_used_value": int.from_bytes(r[12:16], "little"),
        "target_diff": int.from_bytes(r[16:20], "little"),
        "output_digest": int.from_bytes(r[20:24], "little"),
        "defense_error": r[24],
        "hpc_anomaly_byte": r[25],
        "entries": r[26],
        "exits": r[27],
        "const_value_low": r[28],
        "bit_mask_low": r[29],
        "raw": r.hex(),
    }
...
def run_trial(target, args, trial):
    print(f"\n===== Trial {trial} =====")
    print("[test] configure Roulette fault model")
    ...
    print("[test] run masked pipeline")
