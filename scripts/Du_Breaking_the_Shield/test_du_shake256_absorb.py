...
MODEL_MAP = {"none": 0, "abort": 1, "skipblock": 2}
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="SimpleSerial driver for Du SHAKE256 absorb-loop simulation")
    p.add_argument("--hex", required=True)
    p.add_argument("--no-program", action="store_true")
    p.add_argument("--label", default="du-shake256")
    ...
    p.add_argument("--stop-block", type=parse_int, default=4)
    p.add_argument("--skip-block", type=parse_int, default=3)
...
def configure_fault(target, args) -> Dict[str, int]:
    ...
    payload[1] = args.stop_block & 0xff
    payload[2] = args.skip_block & 0xff
    ...
    d = {
        ...
        "absorb_blocks": int.from_bytes(r[8:12], "little"),
        "block_bytes": int.from_bytes(r[12:16], "little"),
...
def read_status(target, timeout, verbose):
    ...
    return {
        ...
        "expected_blocks": int.from_bytes(r[12:16], "little"),
        "used_blocks": int.from_bytes(r[16:20], "little"),
        "skipped_blocks": int.from_bytes(r[20:24], "little"),
...
def run_trial(target, args, trial):
    ...
    if args.model == "none":
        if st["output_matches_clean"] != 1 or st["skipped_blocks"] != 0:
            raise RuntimeError("baseline semantic check failed")
        ...
        if st["output_matches_abort"] != 1:
            raise RuntimeError("abort output does not match abort reference")
        if st["used_blocks"] != st["stop_block"]:
            raise RuntimeError("abort used_blocks != stop_block")
    elif args.model == "skipblock":
        if st["output_matches_skip"] != 1:
            raise RuntimeError("skipblock output does not match skip reference")
        if st["skipped_blocks"] != 1:
            raise RuntimeError("skipblock did not skip exactly one block")
