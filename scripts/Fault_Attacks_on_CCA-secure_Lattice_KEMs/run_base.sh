...
#
# Therefore:
#   accept baseline iff 25 <= target_coeff_cycles <= 25
#
...
python3 "$TEST_SCRIPT" \
  --hex "$HEX_PATH" \
  --label "singlebin-hwdef-baseline-coeff${TARGET_COEFF}" \
  --baud "$BAUD" \
  --trials "$TRIALS" \
  --attack-enable 0 \
...
echo
echo "[done] baseline single-binary hardware-counter test passed"
