...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="SimpleSerial driver for Jendral skipped Keccak absorb simulation")
    p.add_argument("--hex", required=True)
    p.add_argument("--no-program", action="store_true")
    p.add_argument("--label", default="jendral-skip-absorb")
...
def configure_fault(target, args) -> Dict[str, int]:
    ...
    payload[1:5] = u32(args.message_tweak)
    ...
    d = {
        ...
        "target_bytes": int.from_bytes(r[8:12], "little"),
        "target_input_digest": int.from_bytes(r[12:16], "little"),
        "raw": r.hex(),
    }
    if d["ret"] != 0:
...
def read_status(target, timeout, verbose) -> Dict[str, int]:
    ...
    return {
        ...
        "semantic_valid": r[1],
        "seed_matches_clean": r[2],
        "seed_matches_omit": r[3],
        "faults_applied": int.from_bytes(r[4:8], "little"),
        "expected_absorbed_target_bytes": int.from_bytes(r[8:12], "little"),
        "used_absorbed_target_bytes": int.from_bytes(r[12:16], "little"),
        "target_input_digest": int.from_bytes(r[16:20], "little"),
        "prefix_digest": int.from_bytes(r[20:24], "little"),
        "suffix_digest": int.from_bytes(r[24:28], "little"),
        "defense_error": r[28],
...
def read_digest(target, timeout, verbose) -> Dict[str, int]:
    ...
    return {
        "output_seed_digest": int.from_bytes(r[0:4], "little"),
        "clean_seed_digest": int.from_bytes(r[4:8], "little"),
        "omit_seed_digest": int.from_bytes(r[8:12], "little"),
        "output_diff_clean": int.from_bytes(r[12:16], "little"),
    }
...
def run_trial(target, args, trial):
    ...
    print("[test] configure skipped-absorb model")
    ...
    print("[test] run seed-generation")
    ...
    if args.model == "none":
        if st["used_absorbed_target_bytes"] != st["expected_absorbed_target_bytes"]:
            raise RuntimeError("baseline did not absorb all target bytes")
        if st["seed_matches_clean"] != 1:
            raise RuntimeError("baseline seed does not match clean reference")
        if dg is not None and dg["output_diff_clean"] != 0:
            raise RuntimeError(f"baseline output differs from clean reference: {dg['output_diff_clean']}")
    else:
        if st["used_absorbed_target_bytes"] != 0:
            raise RuntimeError("attack did not skip target absorb bytes")
        if st["seed_matches_omit"] != 1:
            raise RuntimeError("attack seed does not match omit-target reference")
        if st["seed_matches_clean"] != 0:
            raise RuntimeError("attack seed unexpectedly matches clean reference")
        if dg is not None and dg["output_diff_clean"] == 0:
            raise RuntimeError("attack output did not differ from clean reference")
