...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="SimpleSerial driver for Krahmer A-fault simulation")
    p.add_argument("--hex", required=True)
    p.add_argument("--no-program", action="store_true")
    p.add_argument("--label", default="krahmer-a-fault")
    ...
    p.add_argument("--target-row", type=parse_int, default=0)
    p.add_argument("--target-col", type=parse_int, default=0)
...
def configure_fault(target, args) -> Dict[str, int]:
    ...
    payload[1] = args.target_row & 0xff
    payload[2] = args.target_col & 0xff
    payload[3:5] = u16(args.target_coeff)
    payload[5:9] = u32(args.fault_value)
    payload[9:13] = u32(args.message_tweak)
    ...
    d = {
        ...
        "target_row": r[2],
        "target_col": r[3],
        "target_coeff": int.from_bytes(r[4:6], "little"),
        "fault_value": int.from_bytes(r[6:10], "little", signed=True),
        "message_tweak": int.from_bytes(r[10:14], "little"),
        "K": r[14],
        "L": r[15],
        "N": int.from_bytes(r[16:20], "little"),
...
def read_status(target, timeout, verbose) -> Dict[str, int]:
    ...
    return {
        ...
        "expected_entry": int.from_bytes(r[12:16], "little", signed=True),
        "used_entry": int.from_bytes(r[16:20], "little", signed=True),
        "expected_block_digest": int.from_bytes(r[20:24], "little"),
        "used_block_digest": int.from_bytes(r[24:28], "little"),
        "defense_error": r[28],
...
def run_trial(target, args, trial):
    ...
    print("[test] configure A-fault model")
    ...
    print("[test] run A expansion + normal consumption")
    ...
    if args.model == "none":
        if st["expected_entry"] != st["used_entry"]:
            raise RuntimeError("baseline target entry changed")
        if st["expected_block_digest"] != st["used_block_digest"]:
            raise RuntimeError("baseline target block changed")
        if dg is not None and dg["output_diff"] != 0:
            raise RuntimeError(f"baseline output differs from reference: {dg['output_diff']}")
    else:
        if st["expected_block_digest"] == st["used_block_digest"]:
            raise RuntimeError("attack did not alter target block")
        if dg is not None and dg["output_diff"] == 0:
