...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="SimpleSerial driver for Krahmer skipping-correction simulation")
    p.add_argument("--hex", required=True)
    p.add_argument("--no-program", action="store_true")
    p.add_argument("--label", default="krahmer-skip-correction")
...
def configure_fault(target, args) -> Dict[str, int]:
    ...
    d = {
        ...
        "target_coeff": int.from_bytes(r[2:4], "little"),
        "message_tweak": int.from_bytes(r[4:8], "little"),
        "ncoeffs": int.from_bytes(r[8:12], "little"),
        "q": int.from_bytes(r[12:16], "little"),
        "raw": r.hex(),
    }
    if d["ret"] != 0:
...
def read_status(target, timeout, verbose) -> Dict[str, int]:
    ...
    return {
        ...
        "target_coeff": int.from_bytes(r[1:3], "little"),
        "semantic_valid": r[3],
        "faults_applied": int.from_bytes(r[4:8], "little"),
        "target_uncorrected": int.from_bytes(r[8:12], "little", signed=True),
        "target_correction": int.from_bytes(r[12:16], "little", signed=True),
        "target_expected": int.from_bytes(r[16:20], "little", signed=True),
        "target_used": int.from_bytes(r[20:24], "little", signed=True),
        "target_error": int.from_bytes(r[24:28], "little", signed=True),
        "defense_error": r[28],
...
def run_trial(target, args, trial):
    ...
    print("[test] configure correction fault model")
    ...
    print("[test] run correction loop")
    ...
    if args.model == "none":
        if st["target_expected"] != st["target_used"]:
            raise RuntimeError("baseline target_used differs from target_expected")
        if dg is not None and dg["output_diff"] != 0:
            raise RuntimeError(f"baseline output differs from reference: {dg['output_diff']}")
    else:
        if st["target_expected"] == st["target_used"]:
            raise RuntimeError("attack did not change target coefficient")
        if st["target_error"] != st["target_correction"]:
            raise RuntimeError(
                f"unexpected target_error: expected correction {st['target_correction']}, got {st['target_error']}"
            )
        if dg is not None and dg["output_diff"] == 0:
