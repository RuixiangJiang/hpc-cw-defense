...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="SimpleSerial driver for Kundu Carry Your Fault simulation")
    p.add_argument("--hex", required=True)
    p.add_argument("--no-program", action="store_true")
    p.add_argument("--label", default="kundu-carry-fault")
    ...
    p.add_argument("--target-bit", type=parse_int, default=7)
...
def configure_fault(target, args) -> Dict[str, int]:
    ...
    payload[4:8] = u32(args.message_tweak)

    r = send_cmd_read(target, "F", "F", 20, args.timeout, payload=bytes(payload), verbose=args.verbose_packets)
    d = {
        "ret": r[0],
        "model": r[1],
        "model_name": MODEL_NAME.get(r[1], f"unknown-{r[1]}"),
        "target_coeff": int.from_bytes(r[4:8], "little"),
        "target_bit": int.from_bytes(r[8:12], "little"),
...
def read_status(target, timeout, verbose) -> Dict[str, int]:
    ...
    return {
        ...
        "target_bit": int.from_bytes(r[12:16], "little"),
        "expected_carry": int.from_bytes(r[16:20], "little"),
        "used_carry": int.from_bytes(r[20:24], "little"),
        "expected_intermediate_bit": r[24],
        "used_intermediate_bit": r[25],
        "defense_error": r[26],
        "hpc_anomaly_byte": r[27],
        "entries": r[28],
        "exits": r[29],
        "raw": r.hex(),
    }
...
def read_detail(target, timeout, verbose) -> Dict[str, int]:
    ...
    return {
        "expected_target_value": int.from_bytes(r[0:4], "little"),
        "used_target_value": int.from_bytes(r[4:8], "little"),
...
def run_trial(target, args, trial):
    ...
    print("[test] configure carry/intermediate fault model")
    ...
    print("[test] run masked-decoding/A2B experiment")
    ...
    if args.model == "none":
        ...
        if dg is not None and dg["output_diff"] != 0:
            raise RuntimeError(f"baseline output differs from reference: {dg['output_diff']}")
        if detail is not None and detail["expected_target_value"] != detail["used_target_value"]:
            raise RuntimeError("baseline target value changed")
    else:
        if st["faults_applied"] != 1:
            raise RuntimeError("attack did not apply exactly one data fault")
        if args.model.startswith("carry") and st["expected_carry"] == st["used_carry"]:
            print("[warn] carry stuck value equals the naturally produced carry; semantic output may be unchanged")
        if args.model.startswith("inter") and st["expected_intermediate_bit"] == st["used_intermediate_bit"]:
            print("[warn] intermediate stuck value equals the naturally produced bit; semantic output may be unchanged")
        if dg is not None and dg["output_diff"] == 0:
            print("[warn] attack output equals reference for this target/value; choose a different target bit or stuck value")
