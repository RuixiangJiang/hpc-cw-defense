...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="SimpleSerial driver for Valsaraj wrong-seed sampler simulation")
    p.add_argument("--hex", required=True)
    p.add_argument("--no-program", action="store_true")
    p.add_argument("--label", default="valsaraj-wrong-seed")
    ...
    p.add_argument("--intended-domain", type=parse_int, default=2)
    p.add_argument("--wrong-domain", type=parse_int, default=1)
...
def configure_fault(target, args) -> Dict[str, int]:
    ...
    payload[1] = args.intended_domain & 0xff
    payload[2] = args.wrong_domain & 0xff
    payload[3:7] = u32(args.message_tweak)
    ...
    d = {
        ...
        "intended_domain": r[2],
        "wrong_domain": r[3],
        "message_tweak": int.from_bytes(r[4:8], "little"),
        "num_domains": int.from_bytes(r[8:12], "little"),
        "seed_bytes": int.from_bytes(r[12:16], "little"),
        "output_coeffs": int.from_bytes(r[16:20], "little"),
        "expected_seed_digest": int.from_bytes(r[20:24], "little"),
        "raw": r.hex(),
    }
    if d["ret"] != 0:
...
def read_status(target, timeout, verbose) -> Dict[str, int]:
    ...
    return {
        ...
        "semantic_valid": r[1],
        "output_matches_clean": r[2],
        "output_matches_offset": r[3],
        "output_matches_wrong": r[4],
        "output_matches_redirect": r[5],
        "expected_domain": r[6],
        "used_domain": r[7],
        "faults_applied": int.from_bytes(r[8:12], "little"),
        "expected_seed_digest": int.from_bytes(r[12:16], "little"),
        "used_seed_digest": int.from_bytes(r[16:20], "little"),
        "base_seed_digest": int.from_bytes(r[20:24], "little"),
        "wrong_seed_digest": int.from_bytes(r[24:28], "little"),
        "defense_error": r[28],
...
def read_digest(target, timeout, verbose) -> Dict[str, int]:
    r = send_cmd_read(target, "D", "D", 20, timeout, verbose=verbose)
    return {
        "output_digest": int.from_bytes(r[0:4], "little"),
        "clean_digest": int.from_bytes(r[4:8], "little"),
        "offset_digest": int.from_bytes(r[8:12], "little"),
        "wrong_digest": int.from_bytes(r[12:16], "little"),
        "redirect_digest": int.from_bytes(r[16:20], "little"),
    }
...
def read_detail(target, timeout, verbose) -> Dict[str, int]:
    ...
    return {
        "redirect_seed_digest": int.from_bytes(r[0:4], "little"),
        "message_tweak": int.from_bytes(r[4:8], "little"),
        "intended_domain": int.from_bytes(r[8:12], "little"),
        "wrong_domain": int.from_bytes(r[12:16], "little"),
    }
...
def run_trial(target, args, trial):
    ...
    print("[test] configure wrong-seed model")
    ...
    print("[test] run sampler on selected seed")
    ...
    if args.model == "none":
        if st["output_matches_clean"] != 1:
            raise RuntimeError("baseline output does not match clean reference")
        if st["expected_seed_digest"] != st["used_seed_digest"]:
            raise RuntimeError("baseline used seed differs from expected seed")
        if dg is not None and dg["output_digest"] != dg["clean_digest"]:
            raise RuntimeError("baseline digest differs from clean digest")
    elif args.model == "offsetskip":
        if st["used_domain"] != 0:
            raise RuntimeError("offsetskip did not use base domain 0")
        if st["used_seed_digest"] != st["base_seed_digest"]:
            raise RuntimeError("offsetskip used seed digest does not match base seed")
        if st["output_matches_offset"] != 1:
            raise RuntimeError("offsetskip output does not match offset-skip reference")
    elif args.model == "wrongdomain":
        if st["used_domain"] != (args.wrong_domain & 0xff):
            raise RuntimeError("wrongdomain did not use requested wrong domain")
        if st["used_seed_digest"] != st["wrong_seed_digest"]:
            raise RuntimeError("wrongdomain used seed digest does not match wrong-domain seed")
        if st["output_matches_wrong"] != 1:
            raise RuntimeError("wrongdomain output does not match wrong-domain reference")
    elif args.model == "redirect":
        if st["used_domain"] != 255:
            raise RuntimeError("redirect did not use redirect domain marker")
        if detail is not None and st["used_seed_digest"] != detail["redirect_seed_digest"]:
            raise RuntimeError("redirect used seed digest does not match redirect seed")
        if st["output_matches_redirect"] != 1:
            raise RuntimeError("redirect output does not match redirect reference")
