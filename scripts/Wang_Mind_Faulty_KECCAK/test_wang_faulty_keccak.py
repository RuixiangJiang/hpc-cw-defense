...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="SimpleSerial driver for Wang Faulty KECCAK simulation")
    p.add_argument("--hex", required=True)
    p.add_argument("--no-program", action="store_true")
    p.add_argument("--label", default="wang-faulty-keccak")
    ...
    p.add_argument("--stop-round", type=parse_int, default=8)
    p.add_argument("--skip-round", type=parse_int, default=7)
...
def configure_fault(target, args) -> Dict[str, int]:
    ...
    payload[1] = args.stop_round & 0xff
    payload[2] = args.skip_round & 0xff
    ...
    d = {
        ...
        "rounds": int.from_bytes(r[8:12], "little"),
        "input_bytes": int.from_bytes(r[12:16], "little"),
        "output_bytes": int.from_bytes(r[16:20], "little"),
...
def read_status(target, timeout, verbose) -> Dict[str, int]:
    ...
    return {
        ...
        "output_matches_abort": r[3],
        "output_matches_skip": r[4],
        "stop_round": r[5],
        "skip_round": r[6],
        "faults_applied": int.from_bytes(r[8:12], "little"),
        "expected_rounds": int.from_bytes(r[12:16], "little"),
        "used_rounds": int.from_bytes(r[16:20], "little"),
        "skipped_rounds": int.from_bytes(r[20:24], "little"),
        "state_digest_after_absorb": int.from_bytes(r[24:28], "little"),
        "defense_error": r[28],
...
def read_digest(target, timeout, verbose) -> Dict[str, int]:
    ...
    return {
        ...
        "abort_digest": int.from_bytes(r[8:12], "little"),
        "skip_digest": int.from_bytes(r[12:16], "little"),
    }
...
def run_trial(target, args, trial):
    ...
    print("[test] configure faulty-keccak model")
    ...
    print("[test] run faulty Keccak routine")
    ...
    if args.model == "none":
        ...
        if st["skipped_rounds"] != 0:
            raise RuntimeError("baseline skipped rounds")
        if dg is not None and dg["output_digest"] != dg["clean_digest"]:
            raise RuntimeError("baseline digest differs from clean digest")
    elif args.model == "abort":
        if st["output_matches_abort"] != 1:
            raise RuntimeError("abort attack output does not match abort reference")
        if st["used_rounds"] != st["stop_round"]:
            raise RuntimeError("abort attack used_rounds != stop_round")
        if st["skipped_rounds"] == 0:
            raise RuntimeError("abort attack did not skip any rounds")
    elif args.model == "skipround":
        if st["output_matches_skip"] != 1:
            raise RuntimeError("skipround attack output does not match skip reference")
        if st["skipped_rounds"] != 1:
            raise RuntimeError("skipround attack did not skip exactly one round")
